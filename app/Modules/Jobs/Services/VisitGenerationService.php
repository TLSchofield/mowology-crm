<?php
/**
 * VisitGenerationService — recurrence/holiday math + visit materialisation.
 *
 * Extracted (2026-06-18, refactor Phase 2) from the procedural Plan function
 * library. The global functions in Plan/VisitGeneration.php now delegate here,
 * so all ~28 callers and the legacy shim are unchanged.
 *
 * Method map (1:1 with the original global functions):
 *   generateVisits()           — orchestrator, DB read/write
 *   getActiveHolidays()        — DB read
 *   findBumpDate()             — PURE (unit-tested)
 *   parseDowList()             — PURE (unit-tested)
 *   calculateRecurrenceDates() — PURE (unit-tested)
 *
 * The three pure methods carry the recurrence logic that feeds the whole
 * schedule and the generate_visits cron; they are the high-value test targets.
 * generateVisits()/getActiveHolidays() still touch the DB and call sibling
 * Plan-domain globals (cleanupOrphanedVisits, ensureCalendarStop,
 * generateVisitNumber, planTimeStringToMinutes, planMinutesToTimeString) which
 * the aggregator loads alongside this service.
 */
class VisitGenerationService
{
     ...
     */
    public static function generateVisits(?int $planId = null, ?int $horizonDays = null): array {
        $db = getDB();
        $plansProcessed = 0;
        $visitsCreated = 0;
        $errors = [];
        ...
        try {
            ...
            $holidays = [];
            try {
                $holidays = self::getActiveHolidays($holidayFromExtended, $holidayTo);
            } catch (Exception $e) {
            ...
            foreach ($plans as $plan) {
                try {
                    ...
                    if ($plan['is_recurring']) {
                        ...
                        $dates = self::calculateRecurrenceDates($plan, $fromDate->format('Y-m-d'), $toDate->format('Y-m-d'), $holidays);
                    } else {
     ...
     */
    public static function getActiveHolidays(string $fromDate, string $toDate): array {
        $db = getDB();
        $holidays = [];
     ...
     */
    public static function findBumpDate(string $holidayDate, array $holidays, array $blackouts): ?string {
     ...
     */
    public static function parseDowList($rawDow, DateTime $fallback): array {
        ...
        while ($current <= $end && count($dates) < $maxDates) {
            ...
            switch ($pattern) {
                ...
                case 'biweekly':
                    ...
                    $targetDays = self::parseDowList($targetDow, $planStart);
            ...
            if ($shouldInclude && isset($holidays[$dateStr])) {
                $bumped = self::findBumpDate($dateStr, $holidays, $blackouts);
