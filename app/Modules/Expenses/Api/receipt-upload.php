...
try {
    ...
    // iOS Vision (on-device Neural Engine) can send pre-extracted text via `vision_text`.
    // When present and non-trivial, use it directly — skip Tesseract + server Vision API.
    // Cleaner text → better ReceiptParser accuracy → better training signal.
    ...
    if ($iosVisionText !== null && strlen($iosVisionText) >= 10) {
        // Use client-supplied on-device Vision text — fastest + cleanest path
