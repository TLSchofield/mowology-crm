...
<?php if ($canEdit): ?>
<script>
// Searchable-select enhancement (mw-searchable). Progressive enhancement: the
// native <select> stays in the DOM (and keeps its name) so the form submits
// exactly as before — we just overlay a filterable combobox with an always-
// available "No contact" clear row. Used for the long contact / company pickers.
(function () {
    'use strict';
    ...
    function enhance(select) {
        ...
        // Build options model from the native <select>.
        var opts = [];
        ...
        function filter(q) {
            ...
            rows.forEach(function (r) {
                // The "No contact" row is always available regardless of query.
    ...
    function init() {
        var list = document.querySelectorAll('select.mw-searchable:not([disabled])');
