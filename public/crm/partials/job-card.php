        ...
        <div class="mw-mc-compact-main">
            ...
            <?php if ($stopStatus !== 'completed' && $stopStatus !== 'skipped'): ?>
                <button type="button" class="mw-mc-compact-route"
                        data-stop-id="<?php echo (int)$stop['stop_id']; ?>"
                        data-address="<?php echo htmlspecialchars($stop['property_address'] ?? ''); ?>"
                        onclick="event.stopPropagation(); (function(sid){ var isMobile=window.matchMedia('(max-width:991px)').matches||('ontouchstart' in window&&window.innerWidth<=991); if(isMobile&&typeof MwNavLauncher!=='undefined'&&typeof MW_ROUTE_STOPS!=='undefined'){var t=null;for(var i=0;i<MW_ROUTE_STOPS.length;i++){if(MW_ROUTE_STOPS[i].stopId===sid){t=MW_ROUTE_STOPS[i];break;}} if(t){MwNavLauncher.launchNavigation(t);return;}} if(typeof MwRouteMap!=='undefined')MwRouteMap.openToStop(sid); })(<?php echo (int)$stop['stop_id']; ?>);">
