...
.cdx-tile:hover:not(.cdx-tile--on-card) {
  background: rgba(45,134,89,.28);
  border-color: var(--cdx-green);
  transform: translateX(3px);
}
.cdx-tile:active:not(.cdx-tile--on-card) { cursor: grabbing; transform: translateX(3px) scale(.98); }
.cdx-tile--on-card { opacity: .38; cursor: default; border-style: dashed; }

.cdx-tile-ico {
  width: 26px; height: 26px;
  background: rgba(127,216,88,.13);
  border-radius: 7px;
...
.cdx-screen-inner { padding: 0 12px 20px; flex: 1; }
...
.cdx-drop-ind.show {
  height: 3px;
  background: var(--cdx-green);
  border-radius: 2px;
  animation: cdx-blink .7s ease infinite;
}
...
.cdx-props-empty {
  display: flex; flex-direction: column;
  align-items: center; justify-content: center;
  height: 100%; color: #C5BFB8;
  font-size: 11px; font-family: var(--cdx-font-mono);
...
.cdx-prop-toggle {
  ...
  padding: 9px 11px; background: #F2EEE8; border-radius: 8px; cursor: pointer;
}
.cdx-tgl-lbl { font-size: 12px; font-weight: 600; color: #333; }
.cdx-tgl {
  width: 34px; height: 19px;
  background: #CCC; border-radius: 10px; position: relative; transition: background .18s;
}
...
.cdx-prop-inp {
  font-family: var(--cdx-font-display); font-size: 12px;
  padding: 8px 11px; border: 1.5px solid #E0DBCF; border-radius: 8px;
...
.cdx-swatch {
  ...
  cursor: pointer; border: 2.5px solid transparent; transition: transform .1s, border-color .1s;
}
...
.cdx-p-svc-badge { padding: 4px 14px; }
...
.cdx-toast {
  ...
  background: var(--cdx-forest); color: #fff;
  font-family: var(--cdx-font-mono); font-size: 12px;
  ...
  <div class="cdx-layout">
    ...
    <aside class="cdx-props">
      <div id="cdxPropsContent">
        <div class="cdx-props-empty" style="height:100%;display:flex;">
...
const CARD_DEFS = {
  stop: {
    label: 'Stop Card',
    defaults: ['stop-time','client-name','address','service-pills','duration','sq-ft','last-visit','photos','clock-in','complete-job'],
    elements: {
      ...
      'service-pills': { label:'Service Pills Row',  icon:'tag',    desc:'Service type badges',          colorable:true  },
  ...
  schedule: {
    ...
    elements: {
      ...
      'service-badge': { label:'Service Badge', icon:'tag',    desc:'Service type chip',           colorable:true  },
...
const COLOR_SWATCHES = [
  { hex:'#2D8659', name:'Mowology Green' },
  { hex:'#1A5F4A', name:'Dark Green' },
  { hex:'#7FD858', name:'Lime' },
  { hex:'#0D3B2E', name:'Forest' },
...
function renderPalette() {
  ...
  const onCard = new Set(state.layouts[state.activeCard]);
  let html = '';
...
function renderCard() {
  const layout = state.layouts[state.activeCard];
  const defs   = CARD_DEFS[state.activeCard].elements;
  ...
  // Build element nodes
  const existing = {};
  dz.querySelectorAll('.cdx-card-el').forEach(el => existing[el.dataset.elemId] = el);
  ...
  layout.forEach((id, i) => {
    ...
    wrap.draggable = true;
    wrap.innerHTML = getPreview(state.activeCard, id) +
      `<button class="cdx-el-rm" title="Remove">×</button>`;
    ...
    wrap.addEventListener('click', e => {
      if (e.target.closest('.cdx-el-rm')) {
        removeElem(id);
      } else {
        state.selectedElem = (state.selectedElem === id) ? null : id;
        render();
      }
    });
    dz.appendChild(wrap);
  ...
  last.className = 'cdx-drop-ind';
  last.dataset.idx = layout.length;
  dz.appendChild(last);
}
...
function renderProps() {
  ...
  if (!state.selectedElem) {
    panel.innerHTML = `<div class="cdx-props-empty" style="height:calc(100vh - 109px);display:flex">
      ${IC.cursor}
  ...
  panel.innerHTML = `
    ...
    <div class="cdx-props-body">
      ...
      <div class="cdx-prop-grp">
        <button class="cdx-btn cdx-btn--ghost" style="width:100%;justify-content:center"
...
function getPreview(cardType, id) {
  if (cardType === 'stop') return getStopPreview(id);
  return getSchedulePreview(id);
}
...
function getSchedulePreview(id) {
  switch(id) {
    ...
    case 'service-badge': return `<div class="cdx-p-svc-badge">
      <span class="cdx-p-badge">🌿 Lawn Mowing</span></div>`;
    case 'duration': return `<div class="cdx-p-meta">
      <div style="width:12px;height:12px">${IC.clock}</div> 90 min</div>`;
    case 'assigned-crew': return `<div class="cdx-p-crew">
      <div class="cdx-p-avatars">
        <div class="cdx-p-cav">TS</div><div class="cdx-p-cav">AK</div></div>
...
function onDZDragOver(e) {
  ...
  let idx = state.layouts[state.activeCard].length;
...
function onDZDrop(e) {
  ...
  if (source === 'palette') {
    if (!layout.includes(elemId)) {
      layout.splice(idx, 0, elemId);
    }
  } else if (source === 'card') {
    const old = layout.indexOf(elemId);
    if (old !== -1) {
      layout.splice(old, 1);
      const ni = idx > old ? idx - 1 : idx;
      layout.splice(ni, 0, elemId);
    }
  }
...
function switchCard(type) {
  ...
  document.querySelectorAll('.cdx-tab').forEach(t => {
    t.classList.toggle('active', t.dataset.card === type);
  });
  render();
}
...
function resetLayout() {
  if (!confirm('Reset to default layout? This will undo your changes.')) return;
  state.layouts[state.activeCard] = [...CARD_DEFS[state.activeCard].defaults];
  state.selectedElem = null;
  state.props[state.activeCard] = {};
  render();
...
function saveLayout() {
  ...
  // POST to endpoint (dummy for now)
  fetch('/crm/api/card-layout.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
  }).catch(() => {}); // Endpoint not yet created — fails silently
  toast('Layout saved ✓');
}
...
function buildExportData() {
  return {
    ...
    layouts: {
      stop:     { order: state.layouts.stop,     props: state.props.stop },
...
(function loadSaved() {
  try {
    ...
    if (d.layouts?.stop?.order)     state.layouts.stop     = d.layouts.stop.order;
    if (d.layouts?.schedule?.order) state.layouts.schedule = d.layouts.schedule.order;
    if (d.layouts?.stop?.props)     state.props.stop       = d.layouts.stop.props;
