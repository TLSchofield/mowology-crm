 ...
 *
 * Usage:
 *   require_once __DIR__ . '/pdf_bootstrap.php';
...
class PdfGenerator
{
    ...
    public function __construct()
    {
        ...
        $this->templatePath = dirname(__DIR__) . '/templates/pdf';
    }
    ...
    {
        // Try repo root / above public_html first
        $abovePublic = dirname(__DIR__, 3);
        if (is_dir($abovePublic . '/storage')) {
            return $abovePublic;
        }

        // Try inside public_html (FTP upload scenario)
        $insidePublic = dirname(__DIR__, 2);
        if (is_dir($insidePublic . '/storage')) {
            return $insidePublic;
        }
