...
$storedName = 'portfolio-' . uniqid() . '.' . $ext;
...
try {
    ...
    error_log('Portfolio upload - DB registration failed: ' . $e->getMessage());
    // Continue — file is uploaded, just not tracked in media system
}
        ...
        if ($processingResult && !empty($processingResult['variants'])) {
            foreach ($processingResult['variants'] as $variantKey => $variant) {
                ...
                try {
                    ...
                    error_log('Portfolio upload - variant registration failed: ' . $e->getMessage());
                }
            }
        }
    } catch (Exception $e) {
        error_log('Portfolio upload - image processing failed: ' . $e->getMessage());
        // Non-fatal — original image still works
    }
}
...
json_response(true, 'Image uploaded and optimized', $responseData);
